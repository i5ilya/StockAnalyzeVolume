...
def list_from_csv_vol(file_v):  # Функция из столбца файла делает список, преобразует в цифры.
    ...
    try:
        for i in range(0, len(list_from_csv_vol)):
            list_from_csv_vol[i] = float(list_from_csv_vol[i])  # преобразуем элементы списка из строк в float
...
# list_crypto_names_clear = list(dict.fromkeys(list_crypto_names))
